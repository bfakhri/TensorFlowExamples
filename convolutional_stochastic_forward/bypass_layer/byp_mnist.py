...
# Returns a random binary vector
def bypass_masks(shape, name="Byp"):
    mask = np.random.randint(2, size=shape)
    mask_inv = 1 - mask 
    mask_var = tf.Variable(mask, name=name, trainable=False)
    mask_inv_var = tf.Variable(mask_inv, name=name, trainable=False)
    return [mask_var, mask_inv_var]
...
with tf.name_scope('MainGraph'):
    ...
    # Some neurons bypass FC2 Directly
    with tf.name_scope('BYP_FC2'):
        byp, stunt = bypass_masks(1024) 
        byp_out = tf.multiply(tf.cast(byp, tf.float32), h_fc1)
    ...
    with tf.name_scope('FC2'):
        ...
        h_fc2 = tf.multiply(tf.cast(stunt, tf.float32), tf.nn.relu(tf.matmul(h_fc1_drop, W_fc2) + b_fc2))

    with tf.name_scope('FC3'):
        W_fc3= weight_variable([1024, 10])
        b_fc3 = bias_variable([10])
        # Dropout Layer
        h_fc2_drop = tf.nn.dropout(tf.add(h_fc2, byp_out), keep_prob)
