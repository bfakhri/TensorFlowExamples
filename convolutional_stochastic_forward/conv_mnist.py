...
with tf.name_scope('MainGraph'):
    ...
    with tf.name_scope('Other'):
        soft_max = tf.nn.softmax(y_pred, name='Softmax')
...
with sess.as_default():
    for cur_step in range(MAX_TRAIN_STEPS):
        ...
        if cur_step % output_steps == 0:
            ...
            # Calculate Stochastic Forward
            p_sum = np.zeros((BATCH_SIZE, 10))
            for i in range(100):
                p = sess.run(soft_max, feed_dict={x: batch[0], y_true: batch[1], keep_prob: 0.5})
                p_sum += p
            p_sum /= 100
            p_diff = p_sum.sum(1) - p_sum.max(1)
            print('Mean Prediction Uncertainty: ', p_diff.mean()*100, '%')
