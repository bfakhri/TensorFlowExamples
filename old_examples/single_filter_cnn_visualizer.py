# This is a version where the Convolutional layers only have 1 filter
# Accuracy takes a hit but it is useful to demonstrate only one filter being
# available (all information goes there)
...
with tf.name_scope('MainGraph'):
    ...
    conv1 = conv_layer(x_image, 1, 1, name='Conv1') 
    conv2 = conv_layer(conv1, 1, 1, name='Conv2') 
    ...
    tf.summary.image('conv1_viz', conv1, 3)
    tf.summary.image('conv2_viz', conv2, 3)
    ...
    with tf.name_scope('FC1'):
        ...
        h_pool2_flat = tf.reshape(conv2, [-1, 7*7])
